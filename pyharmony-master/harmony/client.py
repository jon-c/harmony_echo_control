...
class HarmonyClient(sleekxmpp.ClientXMPP):
    ...
    def get_config(self):
        ...
        jsonStr = json.loads(device_list)
    ...
    def change_channel(self, deviceId,channel):
        """changes the channel.
        Args:
            channel: An int identifying the channel
        Returns:
            nothing
        """
        print deviceId
        for c in channel:
            ...
            cmd = 'action={"type"::"IRCommand","deviceId"::"'+ str(deviceId) + '"'
            cmd  = cmd + ',"command"::"'+c+'"'
            cmd = cmd + '}:status=press'
            print cmd
            action_cmd.text = cmd
            iq_cmd.set_payload(action_cmd)
            ...
            assert len(payload) == 1
            action_cmd = payload[0]
            print action_cmd.text        
            """

    def do_command(self, deviceId,command):
        """changes the channel.
        Args:
            command: An int identifying the channel
        Returns:
            nothing
        """
        print deviceId
